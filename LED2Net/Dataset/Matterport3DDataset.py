...
class Matterport3DDataset(BaseDataset):
    def __init__(self, dataset_image_path, dataset_label_path, mode, shape, image_name, wall_types, aug, camera_height,
                 **kwargs):
        ...
        with open('%s/mp3d_%s.txt' % (dataset_label_path, mode), 'r') as f:
            lst = [x.rstrip().split() for x in f]
    ...
    def __getitem__(self, idx):
        ...
        if aug['stretch']:
            ...
            d, inter = plane_to_depth(self.grid, label['planes'], label['xyz'], label['point_idxs'],
                                      label['cameraHeight'], label['cameraCeilingHeight'])
            inter[:, :, 0] *= kx
            inter[:, :, 1] *= ky
            inter[:, :, 2] *= kz
